...
const { clearDesktopArpEntries, desktopArpTable, desktopDnsCache, desktopGetComputerInfo, desktopGetDnsClientServerAddress, desktopGetEventLog, desktopGetmacTable, desktopGetNetAdapter, desktopGetNetIpConfiguration, desktopGetNetNeighbor, desktopGetNetRoute, desktopGetNetTcpConnection, desktopGetProcess, desktopGetService, desktopHostname, desktopIpconfigAll, desktopNetshInterfaceConfig, desktopNetstatListening, desktopNetstatListeningRows, desktopResolveDnsNameOutput, desktopRoutePrint, desktopScQuery, desktopSystemInfo, desktopTasklist, isDesktopNetshInterfaceConfigCommand, isDesktopRoutePrintCommand, parseDesktopArpCommand, parseDesktopGetComputerInfoCommand, parseDesktopGetDnsClientServerAddressCommand, parseDesktopGetEventLogCommand, parseDesktopGetNetAdapterCommand, parseDesktopGetNetIpConfigurationCommand, parseDesktopGetNetNeighborCommand, parseDesktopGetNetRouteCommand, parseDesktopGetNetTcpConnectionCommand, parseDesktopGetProcessCommand, parseDesktopGetServiceCommand, parseDesktopNetstatCommand, parseDesktopNslookupCommand, parseDesktopPingCommand, parseDesktopRemoteAccessCommand, parseDesktopResolveDnsNameCommand, parseDesktopScCommand, parseDesktopSendMailMessageCommand, parseDesktopTasklistCommand, parseDesktopTestConnectionCommand, parseDesktopTestNetConnectionCommand, parseDesktopTraceCommand, parseDesktopWebRequestCommand } = require(path.join(tmpdir, "engine/desktopDiagnostics.js"));
...
assert(
  ...
  editorSource.includes("systeminfo") &&
  editorSource.includes("Get-ComputerInfo") &&
  ...
  editorSource.includes("Get-NetTCPConnection -State Listen") &&
  editorSource.includes("Get-Process -Id 4080") &&
  editorSource.includes("Get-Service dns") &&
  editorSource.includes("Get-EventLog -LogName Application -Newest 10") &&
  editorSource.includes("sc queryex dns") &&
  editorSource.includes("Get-NetAdapter") &&
  editorSource.includes("Get-NetIPConfiguration -All") &&
  editorSource.includes("Get-DnsClientServerAddress") &&
  editorSource.includes("Get-NetRoute") &&
  editorSource.includes("Get-NetNeighbor") &&
  editorSource.includes("Test-NetConnection www.lab.local -Port 80") &&
  editorSource.includes("Test-Connection www.lab.local -Count 4") &&
  editorSource.includes("Resolve-DnsName www.lab.local") &&
  editorSource.includes("Invoke-WebRequest http://www.lab.local") &&
  editorSource.includes("Send-MailMessage -SmtpServer www.lab.local") &&
  ...
  "Desktop command prompt must expose pathping, netstat PID output, PowerShell process checks, and richer ipconfig diagnostics"
);
...
const systemInfoOutput = desktopSystemInfo(pc);
assert(systemInfoOutput.includes("Host Name") && systemInfoOutput.includes(pc.config.hostname) && systemInfoOutput.includes("Network Card(s)") && systemInfoOutput.includes("IP address(es)"), "Desktop systeminfo helper must include host and NIC inventory evidence");
const computerInfoOutput = desktopGetComputerInfo(pc);
assert(computerInfoOutput.includes("CsName") && computerInfoOutput.includes(pc.config.hostname) && computerInfoOutput.includes("OsName") && computerInfoOutput.includes("CsNetworkAdapters"), "Desktop Get-ComputerInfo helper must include host, OS, and adapter summary evidence");
assert(parseDesktopGetComputerInfoCommand("Get-ComputerInfo").valid, "Desktop Get-ComputerInfo parser must recognize the command");
...
const netAdapterOutput = desktopGetNetAdapter(pc);
assert(netAdapterOutput.includes("InterfaceDescription") && netAdapterOutput.includes("MacAddress") && netAdapterOutput.includes("LinkSpeed") && netAdapterOutput.includes(pc.ports.find((port) => port.kind !== "console").name), "Desktop Get-NetAdapter helper must include adapter status, MAC, and link speed");
const parsedNetAdapter = parseDesktopGetNetAdapterCommand("Get-NetAdapter -Name FastEthernet0");
assert(parsedNetAdapter.valid && parsedNetAdapter.nameFilter === "FastEthernet0", "Desktop Get-NetAdapter parser must accept name filters");
const netIpConfigurationOutput = desktopGetNetIpConfiguration(pc, { all: true });
assert(netIpConfigurationOutput.includes("InterfaceAlias") && netIpConfigurationOutput.includes("IPv4Address") && netIpConfigurationOutput.includes("IPv4DefaultGateway") && netIpConfigurationOutput.includes("DNSServer") && netIpConfigurationOutput.includes("MacAddress"), "Desktop Get-NetIPConfiguration helper must include IPv4, gateway, DNS, and all-mode MAC evidence");
const parsedNetIpConfiguration = parseDesktopGetNetIpConfigurationCommand("Get-NetIPConfiguration -All -InterfaceAlias FastEthernet0");
assert(parsedNetIpConfiguration.valid && parsedNetIpConfiguration.all && parsedNetIpConfiguration.nameFilter === "FastEthernet0", "Desktop Get-NetIPConfiguration parser must accept all and interface-alias filters");
const dnsClientServerAddressOutput = desktopGetDnsClientServerAddress(pc);
assert(dnsClientServerAddressOutput.includes("InterfaceAlias") && dnsClientServerAddressOutput.includes("Address") && dnsClientServerAddressOutput.includes("IPv4") && dnsClientServerAddressOutput.includes(pc.ports.find((port) => port.dnsServer)?.dnsServer), "Desktop Get-DnsClientServerAddress helper must include IPv4 DNS server addresses");
const parsedDnsClientServerAddress = parseDesktopGetDnsClientServerAddressCommand("Get-DnsClientServerAddress -AddressFamily IPv4 -InterfaceAlias FastEthernet0");
assert(parsedDnsClientServerAddress.valid && parsedDnsClientServerAddress.addressFamily === "IPv4" && parsedDnsClientServerAddress.nameFilter === "FastEthernet0", "Desktop Get-DnsClientServerAddress parser must accept address-family and interface-alias filters");
...
const getNetRouteOutput = desktopGetNetRoute(pc);
assert(getNetRouteOutput.includes("DestinationPrefix") && getNetRouteOutput.includes("NextHop") && getNetRouteOutput.includes("0.0.0.0/0") && getNetRouteOutput.includes("ActiveStore"), "Desktop Get-NetRoute helper must include PowerShell-style default and active route rows");
const getNetRouteFilteredOutput = desktopGetNetRoute(pc, { destinationPrefix: "0.0.0.0/0" });
assert(getNetRouteFilteredOutput.includes("0.0.0.0/0") && !getNetRouteFilteredOutput.includes("/32"), "Desktop Get-NetRoute helper must filter by destination prefix");
const parsedGetNetRoute = parseDesktopGetNetRouteCommand("Get-NetRoute -AddressFamily IPv4 -DestinationPrefix 0.0.0.0/0");
assert(parsedGetNetRoute.valid && parsedGetNetRoute.addressFamily === "IPv4" && parsedGetNetRoute.destinationPrefix === "0.0.0.0/0", "Desktop Get-NetRoute parser must accept address-family and destination-prefix filters");
...
const getNetNeighborOutput = desktopGetNetNeighbor(pcWithArp);
assert(getNetNeighborOutput.includes("LinkLayerAddress") && getNetNeighborOutput.includes("Reachable") && getNetNeighborOutput.includes("192.168.10.1") && getNetNeighborOutput.includes("02-00-00-00-00-01"), "Desktop Get-NetNeighbor helper must include PowerShell-style neighbor rows");
const getNetNeighborFilteredOutput = desktopGetNetNeighbor(pcWithArp, { ipAddress: "192.168.10.2" });
assert(getNetNeighborFilteredOutput.includes("192.168.10.2") && !getNetNeighborFilteredOutput.includes("192.168.10.1"), "Desktop Get-NetNeighbor helper must filter by IP address");
const parsedGetNetNeighbor = parseDesktopGetNetNeighborCommand("Get-NetNeighbor -AddressFamily IPv4 -IPAddress 192.168.10.1");
assert(parsedGetNetNeighbor.valid && parsedGetNetNeighbor.addressFamily === "IPv4" && parsedGetNetNeighbor.ipAddress === "192.168.10.1", "Desktop Get-NetNeighbor parser must accept address-family and IP filters");
...
const getNetTcpOutput = desktopGetNetTcpConnection(server, { state: "Listen" });
assert(getNetTcpOutput.includes("OwningProcess") && getNetTcpOutput.includes("LocalPort") && getNetTcpOutput.includes("4080") && getNetTcpOutput.includes("Listen") && !getNetTcpOutput.includes("4053"), "Desktop Get-NetTCPConnection helper must list TCP listener owning-process evidence without UDP listeners");
const getNetTcpPortOutput = desktopGetNetTcpConnection(server, { localPort: "80" });
assert(getNetTcpPortOutput.includes("4080") && !getNetTcpPortOutput.includes("4021"), "Desktop Get-NetTCPConnection helper must filter by local port");
const parsedGetNetTcp = parseDesktopGetNetTcpConnectionCommand("Get-NetTCPConnection -State Listen -LocalPort 80");
assert(parsedGetNetTcp.valid && parsedGetNetTcp.state === "Listen" && parsedGetNetTcp.localPort === "80", "Desktop Get-NetTCPConnection parser must accept state and local-port filters");
const parsedGetNetTcpColon = parseDesktopGetNetTcpConnectionCommand("Get-NetTCPConnection -State:Listen -LocalPort:80");
assert(parsedGetNetTcpColon.valid && parsedGetNetTcpColon.state === "Listen" && parsedGetNetTcpColon.localPort === "80", "Desktop Get-NetTCPConnection parser must accept PowerShell colon option values");
const getProcessOutput = desktopGetProcess(server, { pidFilter: "4080" });
assert(getProcessOutput.includes("ProcessName") && getProcessOutput.includes("4080") && getProcessOutput.includes("ptweb-http") && !getProcessOutput.includes("4053"), "Desktop Get-Process helper must filter process rows by PID");
const parsedGetProcess = parseDesktopGetProcessCommand("Get-Process -Id 4080");
assert(parsedGetProcess.valid && parsedGetProcess.pidFilter === "4080", "Desktop Get-Process parser must accept -Id filters");
const parsedGetProcessAlias = parseDesktopGetProcessCommand("gps -Id:4080");
assert(parsedGetProcessAlias.valid && parsedGetProcessAlias.pidFilter === "4080", "Desktop Get-Process parser must accept common aliases and colon option values");
const getServiceOutput = desktopGetService(server, { nameFilter: "dns" });
assert(getServiceOutput.includes("Status") && getServiceOutput.includes("Running") && getServiceOutput.includes("dns") && getServiceOutput.includes("PTWeb DNS Service") && !getServiceOutput.includes("PTWeb HTTP Service"), "Desktop Get-Service helper must filter service state rows by name");
const serverWithStoppedService = { ...server, config: { ...server.config, services: { ...server.config.services, ftp: false } } };
const getServiceAllOutput = desktopGetService(serverWithStoppedService);
assert(getServiceAllOutput.includes("Running") && getServiceAllOutput.includes("Stopped"), "Desktop Get-Service helper must include running and stopped service states");
const parsedGetService = parseDesktopGetServiceCommand("Get-Service -Name dns");
assert(parsedGetService.valid && parsedGetService.nameFilter === "dns", "Desktop Get-Service parser must accept name filters");
const parsedGetServiceAlias = parseDesktopGetServiceCommand("gsv dns");
assert(parsedGetServiceAlias.valid && parsedGetServiceAlias.nameFilter === "dns", "Desktop Get-Service parser must accept gsv alias and positional filters");
const serverWithLogs = { ...server, runtime: { ...server.runtime, logs: [{ id: "log1", level: "info", message: "DNS QUERY www.lab.local from PC0: 192.168.10.10", createdAt: 1780000000000 }, { id: "log2", level: "warning", message: "HTTP GET from PC0 failed", createdAt: 1780000001000 }] } };
const eventLogOutput = desktopGetEventLog(serverWithLogs, { newest: 2 });
assert(eventLogOutput.includes("TimeGenerated") && eventLogOutput.includes("Information") && eventLogOutput.includes("Warning") && eventLogOutput.includes("DNS QUERY") && eventLogOutput.includes("HTTP GET"), "Desktop Get-EventLog helper must render recent runtime logs with event types and sources");
const eventLogFilteredOutput = desktopGetEventLog(serverWithLogs, { newest: 5, sourceFilter: "dns" });
assert(eventLogFilteredOutput.includes("DNS QUERY") && !eventLogFilteredOutput.includes("HTTP GET"), "Desktop Get-EventLog helper must filter events by source/message");
const parsedGetEventLog = parseDesktopGetEventLogCommand("Get-EventLog -LogName Application -Newest 5 -Source DNS");
assert(parsedGetEventLog.valid && parsedGetEventLog.newest === 5 && parsedGetEventLog.sourceFilter === "DNS", "Desktop Get-EventLog parser must accept newest and source filters");
const parsedGetWinEvent = parseDesktopGetEventLogCommand("Get-WinEvent -LogName Application -MaxEvents:60 -ProviderName:HTTP");
assert(parsedGetWinEvent.valid && parsedGetWinEvent.newest === 50 && parsedGetWinEvent.sourceFilter === "HTTP", "Desktop Get-WinEvent parser must accept max-events/provider aliases and clamp counts");
...
const parsedTestConnection = parseDesktopTestConnectionCommand("Test-Connection -ComputerName www.lab.local -Count 3");
assert(parsedTestConnection.valid && parsedTestConnection.targetText === "www.lab.local" && parsedTestConnection.count === 3, "Desktop Test-Connection parser must accept computer-name and count options");
const parsedTestConnectionColon = parseDesktopTestConnectionCommand("Test-Connection www.lab.local -Count:12 -IPv4");
assert(parsedTestConnectionColon.valid && parsedTestConnectionColon.targetText === "www.lab.local" && parsedTestConnectionColon.count === 10, "Desktop Test-Connection parser must accept positional target, colon count, and clamp request counts");
...
const parsedResolveDnsName = parseDesktopResolveDnsNameCommand("Resolve-DnsName -Name www.lab.local -Server 192.168.10.10 -Type A");
assert(parsedResolveDnsName.valid && parsedResolveDnsName.name === "www.lab.local" && parsedResolveDnsName.serverText === "192.168.10.10" && parsedResolveDnsName.queryType === "A", "Desktop Resolve-DnsName parser must accept named server and type options");
const parsedResolveDnsNamePositional = parseDesktopResolveDnsNameCommand("Resolve-DnsName 192.168.10.10 -Type:PTR");
assert(parsedResolveDnsNamePositional.valid && parsedResolveDnsNamePositional.name === "192.168.10.10" && parsedResolveDnsNamePositional.queryType === "PTR", "Desktop Resolve-DnsName parser must accept positional names and colon type options");
const parsedWebRequest = parseDesktopWebRequestCommand("Invoke-WebRequest -Uri http://www.lab.local -UseBasicParsing");
assert(parsedWebRequest.valid && parsedWebRequest.targetText === "http://www.lab.local" && parsedWebRequest.method === "GET", "Desktop Invoke-WebRequest parser must accept URI and ignored compatibility switches");
const parsedWebRequestAlias = parseDesktopWebRequestCommand("iwr http://www.lab.local -Method:GET");
assert(parsedWebRequestAlias.valid && parsedWebRequestAlias.targetText === "http://www.lab.local" && parsedWebRequestAlias.method === "GET", "Desktop iwr parser must accept positional URI and colon method options");
const parsedSendMailMessage = parseDesktopSendMailMessageCommand("Send-MailMessage -SmtpServer www.lab.local -To admin@lab.local -Body hello");
assert(parsedSendMailMessage.valid && parsedSendMailMessage.serverText === "www.lab.local" && parsedSendMailMessage.recipient === "admin@lab.local" && parsedSendMailMessage.message === "hello", "Desktop Send-MailMessage parser must accept SMTP server, recipient, and body options");
const parsedSendMailMessageColon = parseDesktopSendMailMessageCommand("Send-MailMessage -SmtpServer:www.lab.local -To:admin@lab.local -Subject:check");
assert(parsedSendMailMessageColon.valid && parsedSendMailMessageColon.serverText === "www.lab.local" && parsedSendMailMessageColon.recipient === "admin@lab.local" && parsedSendMailMessageColon.message === "check", "Desktop Send-MailMessage parser must accept colon option values");
const resolveDnsNameOutput = desktopResolveDnsNameOutput({ queryName: "www.lab.local", queryType: "A", recordValue: "192.168.10.10", dnsServerIp: "192.168.10.10", serverLabel: "Server0" });
assert(resolveDnsNameOutput.includes("Server") && resolveDnsNameOutput.includes("IPAddress") && resolveDnsNameOutput.includes("www.lab.local") && resolveDnsNameOutput.includes("192.168.10.10"), "Desktop Resolve-DnsName output helper must render PowerShell-style A answer rows");
const resolveDnsNamePtrOutput = desktopResolveDnsNameOutput({ queryName: "192.168.10.10", queryType: "PTR", recordValue: "www.lab.local", dnsServerIp: "192.168.10.10", serverLabel: "Server0" });
assert(resolveDnsNamePtrOutput.includes("NameHost") && resolveDnsNamePtrOutput.includes("in-addr.arpa") && resolveDnsNamePtrOutput.includes("www.lab.local"), "Desktop Resolve-DnsName output helper must render PTR answer rows");
...
assert(verificationPlanText.includes("Verify desktop identity") && verificationPlanText.includes("Desktop > Command Prompt > systeminfo") && verificationPlanText.includes("Desktop > Command Prompt > Get-ComputerInfo") && verificationPlanText.includes("Desktop systeminfo/Get-ComputerInfo show host identity and OS inventory") && verificationPlanText.includes("Desktop > Command Prompt > getmac") && verificationPlanText.includes("Desktop > Command Prompt > getmac /v") && verificationPlanText.includes("Desktop > Command Prompt > netsh interface ip show config") && verificationPlanText.includes("Desktop > Command Prompt > Get-NetAdapter") && verificationPlanText.includes("Desktop > Command Prompt > Get-NetIPConfiguration -All") && verificationPlanText.includes("Desktop > Command Prompt > Get-DnsClientServerAddress") && verificationPlanText.includes("Desktop > Command Prompt > Get-NetRoute") && verificationPlanText.includes("Desktop > Command Prompt > Get-NetNeighbor") && verificationPlanText.includes("PowerShell adapter commands show interface status, IPv4 address, gateway, and DNS server evidence") && verificationPlanText.includes("PowerShell route and neighbor commands show active route and ARP evidence") && verificationPlanText.includes("Desktop > Command Prompt > route print") && verificationPlanText.includes("Desktop > Command Prompt > route print -4"), "verification plan text must include Desktop identity and adapter evidence checks");
assert(verificationPlanText.includes("Desktop > Command Prompt > Test-Connection <server> -Count 4") && verificationPlanText.includes("Desktop Test-Connection reports expected ICMP replies") && verificationPlanText.includes("Desktop > Command Prompt > Test-NetConnection <server> -Port <port>") && verificationPlanText.includes("Desktop > Command Prompt > netstat -an") && verificationPlanText.includes("Desktop > Command Prompt > netstat -ano") && verificationPlanText.includes("Desktop > Command Prompt > netstat -abno") && verificationPlanText.includes("Desktop > Command Prompt > tasklist /svc") && verificationPlanText.includes("Desktop > Command Prompt > Get-NetTCPConnection -State Listen") && verificationPlanText.includes("Desktop > Command Prompt > Get-Process -Id <pid>") && verificationPlanText.includes("Desktop > Command Prompt > Get-Service <service>") && verificationPlanText.includes("Desktop > Command Prompt > Get-EventLog -LogName Application -Newest 10") && verificationPlanText.includes("Desktop > Command Prompt > sc queryex <service>") && verificationPlanText.includes("Desktop Get-Service shows service running/stopped state") && verificationPlanText.includes("Desktop Get-EventLog shows recent runtime service events") && verificationPlanText.includes("Desktop > Command Prompt > nslookup <record> [dns-server]") && verificationPlanText.includes("Desktop > Command Prompt > Resolve-DnsName <record> -Server <dns-server> -Type A") && verificationPlanText.includes("Desktop Resolve-DnsName returns A/PTR answer records when DNS is reachable") && verificationPlanText.includes("Desktop > Command Prompt > Invoke-WebRequest http://<server>") && verificationPlanText.includes("Desktop Invoke-WebRequest returns StatusCode 200 when HTTP is reachable") && verificationPlanText.includes("Desktop > Command Prompt > Send-MailMessage -SmtpServer <server> -To <recipient>") && verificationPlanText.includes("Desktop Send-MailMessage reports accepted EMAIL delivery when reachable") && verificationPlanText.includes("Desktop Test-NetConnection reports expected TcpTestSucceeded state") && verificationPlanText.includes("Desktop netstat shows expected listening service ports") && verificationPlanText.includes("Desktop netstat -ano shows listener PID evidence") && verificationPlanText.includes("Desktop netstat -abno shows listener process names") && verificationPlanText.includes("Desktop tasklist /svc maps listener PIDs to service process names") && verificationPlanText.includes("Desktop Get-NetTCPConnection shows TCP listener owning-process evidence") && verificationPlanText.includes("Desktop Get-Process -Id confirms listener process identity") && verificationPlanText.includes("Desktop sc queryex shows service state and PID evidence") && verificationPlanText.includes("show services summary") && verificationPlanText.includes("show service logs summary") && verificationPlanText.includes("show ip dhcp pool summary") && verificationPlanText.includes("show ip dhcp binding summary") && verificationPlanText.includes("show hosts summary") && verificationPlanText.includes("show service logs dns") && verificationPlanText.includes("show service logs http"), "verification plan text must include service-specific Desktop, directed DNS, Resolve-DnsName, Invoke-WebRequest, Send-MailMessage, Test-Connection, service logs, PowerShell process/service, and log checks for service devices");
...
assert(instructorWorkbookText.includes("ipconfig /all, netsh interface ip show config, Get-NetAdapter, Get-NetIPConfiguration -All, or Get-DnsClientServerAddress") && instructorWorkbookText.includes("route print, Get-NetRoute, arp -a, or Get-NetNeighbor") && instructorWorkbookText.includes("Test-Connection <server> -Count 4") && instructorWorkbookText.includes("Invoke-WebRequest http://<server>") && instructorWorkbookText.includes("Send-MailMessage -SmtpServer <server> -To <recipient>") && instructorWorkbookText.includes("Test-NetConnection <server> -Port <port>") && instructorWorkbookText.includes("Get-EventLog -LogName Application -Newest 10") && instructorWorkbookText.includes("Desktop Command Prompt netstat -an, netstat -ano, or netstat -abno plus tasklist /svc, Get-NetTCPConnection -State Listen, Get-Process -Id <pid>, Get-Service <service>, and sc queryex <service>") && instructorWorkbookText.includes("PID evidence") && instructorWorkbookText.includes("process names") && instructorWorkbookText.includes("owning processes") && instructorWorkbookText.includes("service state") && instructorWorkbookText.includes("nslookup <record> <dns-server> or Resolve-DnsName <record> -Server <dns-server> -Type A") && instructorWorkbookText.includes("show services summary") && instructorWorkbookText.includes("show ip dhcp pool summary") && instructorWorkbookText.includes("show ip dhcp binding summary") && instructorWorkbookText.includes("show hosts summary") && instructorWorkbookText.includes("show service logs dns") && instructorWorkbookText.includes("show service logs summary"), "lab workbook must recommend Desktop netsh, PowerShell adapter/route/neighbor, Test-Connection, Invoke-WebRequest, Send-MailMessage, Test-NetConnection, listener/PID/process/state, PowerShell process/service/logs, directed DNS, Resolve-DnsName, service, DHCP, DNS, and log summary checks");
