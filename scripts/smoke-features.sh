...
const { clearDesktopArpEntries, desktopArpTable, desktopDnsCache, desktopGetmacTable, desktopHostname, desktopIpconfigAll, desktopNetshInterfaceConfig, desktopNetstatListening, desktopNetstatListeningRows, desktopRoutePrint, desktopTasklist, isDesktopNetshInterfaceConfigCommand, isDesktopRoutePrintCommand, parseDesktopArpCommand, parseDesktopNetstatCommand, parseDesktopNslookupCommand, parseDesktopPingCommand, parseDesktopRemoteAccessCommand, parseDesktopTasklistCommand, parseDesktopTraceCommand } = require(path.join(tmpdir, "engine/desktopDiagnostics.js"));
...
assert(serviceReachabilityText.includes("Service Reachability Report") && serviceReachabilityText.includes("Service Status Summary") && serviceReachabilityText.includes("Service Log Summary") && serviceReachabilityText.includes("| DNS") && serviceReachabilityText.includes("Listening Ports") && serviceReachabilityText.includes(":80") && serviceReachabilityText.includes("PID") && serviceReachabilityText.includes("4080") && serviceReachabilityText.includes("Checks"), "service reachability text must include service summary, service logs, listener PID, and check output");
...
assert(verificationPlanText.includes("Desktop > Command Prompt > netstat -an") && verificationPlanText.includes("Desktop > Command Prompt > netstat -ano") && verificationPlanText.includes("Desktop > Command Prompt > tasklist /svc") && verificationPlanText.includes("Desktop > Command Prompt > nslookup <record> [dns-server]") && verificationPlanText.includes("Desktop netstat shows expected listening service ports") && verificationPlanText.includes("Desktop netstat -ano shows listener PID evidence") && verificationPlanText.includes("Desktop tasklist /svc maps listener PIDs to service process names") && verificationPlanText.includes("show services summary") && verificationPlanText.includes("show service logs summary") && verificationPlanText.includes("show ip dhcp pool summary") && verificationPlanText.includes("show ip dhcp binding summary") && verificationPlanText.includes("show hosts summary") && verificationPlanText.includes("show service logs dns") && verificationPlanText.includes("show service logs http"), "verification plan text must include service-specific Desktop, directed DNS, service, and log checks for service devices");
...
assert(instructorWorkbookText.includes("netsh interface ip show config") && instructorWorkbookText.includes("Desktop Command Prompt netstat -an or netstat -ano plus tasklist /svc") && instructorWorkbookText.includes("PID evidence") && instructorWorkbookText.includes("nslookup <record> <dns-server>") && instructorWorkbookText.includes("show services summary") && instructorWorkbookText.includes("show ip dhcp pool summary") && instructorWorkbookText.includes("show ip dhcp binding summary") && instructorWorkbookText.includes("show hosts summary") && instructorWorkbookText.includes("show service logs dns") && instructorWorkbookText.includes("show service logs summary"), "lab workbook must recommend Desktop netsh, listener/PID, directed DNS, service, DHCP, DNS, and log summary checks");
...
assert(projectReportText.includes("Service Summary") && projectReportText.includes("Enabled services") && projectReportText.includes("Runtime logs") && projectReportText.includes("Service Log Summary") && projectReportText.includes("Listening Ports") && projectReportText.includes(":80") && projectReportText.includes("PID") && projectReportText.includes("4080") && projectReportText.includes("| Device") && projectReportText.includes("| DNS"), "project report services section must include service, listener PID, and DNS log summary rows");
