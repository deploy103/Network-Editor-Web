...
import { analyzeRoutingMatrix } from "./routingMatrix";
...
function deviceInventorySection(project: NetworkProject): ReportSection {
  const rows = project.devices.map((device) => {
    ...
    const connectedPorts = device.ports.filter((port) => port.linkId).length;
    ...
    return [
      ...
      `${connectedPorts}/${device.ports.length}`,
...
function capacityPlanSection(project: NetworkProject): ReportSection {
  ...
  return {
    title: "Capacity Plan",
    lines: [
      `Ports ${capacity.totals.portsConnected}/${capacity.totals.portsTotal} connected, modules ${capacity.totals.modulesInstalled}/${capacity.totals.moduleSlots}, warnings ${capacity.totals.warnings}`,
      "",
      "Device Capacity",
      ...table(["Device", "Kind", "Ports", "Modules", "Routes", "Policy", "Warnings"], deviceRows),
      "",
      "DHCP Capacity",
...
function routingSection(project: NetworkProject): ReportSection {
  ...
  return {
    ...
    lines: [
      "Static Routes",
      ...table(["Device", "Network", "Mask", "Next hop", "Distance", "Track"], staticRoutes),
      "",
      "Dynamic Routing",
...
function serviceSection(project: NetworkProject): ReportSection {
  const serviceRows = project.devices
    .filter((device) => enabledServices(device).length || device.config.dhcpPools.length || device.config.dnsRecords.length || device.config.nameServers.length)
    .map((device) => [
      device.label,
...
function securityMatrixSection(project: NetworkProject): ReportSection {
  ...
  return {
    ...
    lines: [
      `Zones ${matrix.totals.zones}, ACL ${matrix.totals.aclRules}, NAT ${matrix.totals.natRules}, PBR ${matrix.totals.pbrRules}, warnings ${matrix.totals.warnings}`,
      "",
      "Policy Matrix",
