...
function addressingTasks(project: NetworkProject): VerificationTask[] {
  ...
  const desktopTasks = project.devices
    ...
    .map((device) => task("desktop", "recommended", `Verify desktop identity on ${device.label}`, {
      ...
      commands: ["Desktop > Command Prompt > hostname", "Desktop > Command Prompt > systeminfo", "Desktop > Command Prompt > Get-ComputerInfo", "Desktop > Command Prompt > getmac", "Desktop > Command Prompt > getmac /v", "Desktop > Command Prompt > ipconfig /all", "Desktop > Command Prompt > netsh interface ip show config", "Desktop > Command Prompt > Get-NetAdapter", "Desktop > Command Prompt > Get-NetIPConfiguration -All", "Desktop > Command Prompt > Get-DnsClientServerAddress", "Desktop > Command Prompt > Get-NetRoute", "Desktop > Command Prompt > Get-NetNeighbor", "Desktop > Command Prompt > route print", "Desktop > Command Prompt > route print -4"],
      expected: [
        `${device.config.hostname || device.label} hostname visible`,
        "Desktop systeminfo/Get-ComputerInfo show host identity and OS inventory",
...
function serviceVerificationCommands(device: NetworkDevice): string[] {
  const commands = ["Desktop > Services", "Desktop > Command Prompt > Test-Connection <server> -Count 4", "Desktop > Command Prompt > Test-NetConnection <server> -Port <port>", "Desktop > Command Prompt > netstat -an", "Desktop > Command Prompt > netstat -ano", "Desktop > Command Prompt > netstat -abno", "Desktop > Command Prompt > tasklist /svc", "Desktop > Command Prompt > Get-NetTCPConnection -State Listen", "Desktop > Command Prompt > Get-Process -Id <pid>", "Desktop > Command Prompt > Get-Service <service>", "Desktop > Command Prompt > Get-EventLog -LogName Application -Newest 10", "Desktop > Command Prompt > sc queryex <service>", "show services", "show services summary", "show service logs", "show service logs summary"];
...
function serviceVerificationExpected(device: NetworkDevice): string[] {
  ...
  if (expected.length) expected.push("Desktop Test-Connection reports expected ICMP replies", "Desktop Test-NetConnection reports expected TcpTestSucceeded state", "Desktop netstat shows expected listening service ports", "Desktop netstat -ano shows listener PID evidence", "Desktop netstat -abno shows listener process names", "Desktop tasklist /svc maps listener PIDs to service process names", "Desktop Get-NetTCPConnection shows TCP listener owning-process evidence", "Desktop Get-Process -Id confirms listener process identity", "Desktop Get-Service shows service running/stopped state", "Desktop Get-EventLog shows recent runtime service events", "Desktop sc queryex shows service state and PID evidence");
