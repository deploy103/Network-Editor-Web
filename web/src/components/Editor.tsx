...
const desktopQuickCommands = ["help", "hostname", "getmac", "getmac /v", "ipconfig /all", "ipconfig /displaydns", "ipconfig /flushdns", "ipconfig /renew", "ipconfig /release", "netsh interface ip show config", "Get-NetAdapter", "Get-NetIPConfiguration -All", "Get-DnsClientServerAddress", "Get-NetRoute", "Get-NetNeighbor", "arp -a", "arp -d *", "route print", "route print -4", "netstat -r", "netstat -rn", "netstat -an", "netstat -ano", "netstat -abno", "tasklist /svc", "Get-NetTCPConnection -State Listen", "Get-Process -Id 4080", "Get-Service dns", "sc queryex dns", "Test-NetConnection www.lab.local -Port 80", "Test-Connection www.lab.local -Count 4", "ping -n 4 www.lab.local", "tracert www.lab.local", "pathping www.lab.local", "nslookup www.lab.local", "Resolve-DnsName www.lab.local", "Invoke-WebRequest http://www.lab.local", "web www.lab.local", "ftp www.lab.local", "mail www.lab.local admin@lab.local test", "ssh 192.168.1.1", "telnet 192.168.1.1", "tftp www.lab.local", "syslog www.lab.local link-check"];
...
async function desktopCommand(project: NetworkProject, device: NetworkDevice, command: string, onProjectChange: (project: NetworkProject, message: string) => void): Promise<string> {
  ...
  if (lower === "help" || lower === "?") {
    return [
      ...
      "  Get-NetAdapter [-Name adapter] | Get-NetIPConfiguration [-All] [-InterfaceAlias adapter] | Get-DnsClientServerAddress [-AddressFamily IPv4]",
      "  netsh interface ip show config | Get-NetRoute [-DestinationPrefix prefix] | Get-NetNeighbor [-IPAddress ip] | arp -a | arp -d <ip|*> | route print [-4] | netstat -r|-rn | netstat -an|-ano|-abno | tasklist [/svc] [/fi \"PID eq <pid>\"]",
      "  Get-NetTCPConnection [-State Listen] [-LocalPort port] | Get-Process [-Id pid|-Name name] | Get-Service [service] | sc query[ex] [service]",
  ...
  if (isDesktopNetshInterfaceConfigCommand(command)) return desktopNetshInterfaceConfig(device);
  const getNetAdapterCommand = parseDesktopGetNetAdapterCommand(command);
  if (getNetAdapterCommand.valid) return desktopGetNetAdapter(device, { nameFilter: getNetAdapterCommand.nameFilter });
  const getNetIpConfigurationCommand = parseDesktopGetNetIpConfigurationCommand(command);
  if (getNetIpConfigurationCommand.valid) return desktopGetNetIpConfiguration(device, { all: getNetIpConfigurationCommand.all, nameFilter: getNetIpConfigurationCommand.nameFilter });
  const getDnsClientServerAddressCommand = parseDesktopGetDnsClientServerAddressCommand(command);
  if (getDnsClientServerAddressCommand.valid) return desktopGetDnsClientServerAddress(device, { addressFamily: getDnsClientServerAddressCommand.addressFamily, nameFilter: getDnsClientServerAddressCommand.nameFilter });
  ...
  const getNetRouteCommand = parseDesktopGetNetRouteCommand(command);
  if (getNetRouteCommand.valid) return desktopGetNetRoute(device, { addressFamily: getNetRouteCommand.addressFamily, destinationPrefix: getNetRouteCommand.destinationPrefix });
  const getNetNeighborCommand = parseDesktopGetNetNeighborCommand(command);
  if (getNetNeighborCommand.valid) return desktopGetNetNeighbor(device, { addressFamily: getNetNeighborCommand.addressFamily, ipAddress: getNetNeighborCommand.ipAddress });
  ...
  const getNetTcpConnectionCommand = parseDesktopGetNetTcpConnectionCommand(command);
  if (getNetTcpConnectionCommand.valid) return desktopGetNetTcpConnection(device, { state: getNetTcpConnectionCommand.state, localPort: getNetTcpConnectionCommand.localPort });
  const getProcessCommand = parseDesktopGetProcessCommand(command);
  if (getProcessCommand.valid) return desktopGetProcess(device, { pidFilter: getProcessCommand.pidFilter, nameFilter: getProcessCommand.nameFilter });
  const getServiceCommand = parseDesktopGetServiceCommand(command);
  if (getServiceCommand.valid) return desktopGetService(device, { nameFilter: getServiceCommand.nameFilter });
  const scCommand = parseDesktopScCommand(command);
  if (scCommand.valid) return desktopScQuery(device, { extended: scCommand.extended, serviceName: scCommand.serviceName });
  ...
  const testConnection = parseDesktopTestConnectionCommand(command);
  if (testConnection.valid) {
    if (!testConnection.targetText.trim()) return "사용법: Test-Connection <ip|이름> [-Count 횟수]";
    const resolved = await resolveDesktopNetworkTarget(project, device, testConnection.targetText, onProjectChange);
    if (!resolved.target) return `Test-Connection : ${resolved.error}`;
    let nextProject = resolved.project;
    const targetAddress = primaryDeviceIp(resolved.target) || testConnection.targetText.trim();
    const rows: string[] = [];
    let received = 0;
    for (let index = 0; index < testConnection.count; index += 1) {
      const result = await simulatePing(nextProject, device.id, resolved.target.id);
      nextProject = result.project;
      if (result.success) {
        received += 1;
        rows.push(`${desktopHostname(device).padEnd(14)} ${resolved.target.label.padEnd(18)} ${targetAddress.padEnd(15)} ${"32".padStart(5)} ${"<1".padStart(8)}`);
      }
    }
    onProjectChange(nextProject, received === testConnection.count ? `Test-Connection ${resolved.target.label} 성공 (${received}/${testConnection.count}).` : `Test-Connection ${resolved.target.label} 손실 ${testConnection.count - received}/${testConnection.count}.`);
    if (!rows.length) return `Test-Connection : Testing connection to computer '${testConnection.targetText.trim()}' failed.`;
    return [
  ...
  const resolveDnsName = parseDesktopResolveDnsNameCommand(command);
  if (resolveDnsName.valid) {
    const name = cleanHost(resolveDnsName.name);
    if (!name) return "사용법: Resolve-DnsName <이름|ip> [-Server dns] [-Type A|PTR]";
    const requestedType = resolveDnsName.queryType.toUpperCase();
    if (requestedType && requestedType !== "A" && requestedType !== "PTR") {
      return `Resolve-DnsName : Query type ${requestedType} is not supported. Use A or PTR.`;
    }
    let dnsServerIp = resolveDnsName.serverText ? cleanHost(resolveDnsName.serverText) : (device.ports.find((port) => port.dnsServer)?.dnsServer ?? "");
    if (dnsServerIp && !isIpv4(dnsServerIp)) {
      dnsServerIp = primaryDeviceIp(resolveDesktopTarget(project, dnsServerIp) ?? undefined) || "";
    }
    if (!dnsServerIp) return "Resolve-DnsName : DNS server is not configured.";
    const server = project.devices.find((item) => item.config.services.dns && item.ports.some((port) => port.ipAddress === dnsServerIp));
    if (!server) return `Resolve-DnsName : DNS server ${dnsServerIp} was not found.`;
    const queryType = requestedType === "PTR" || (!requestedType && isIpv4(name)) ? "PTR" : "A";
    const reachability = await simulatePing(project, device.id, server.id, "dns");
    if (!reachability.success) {
      const nextProject = appendDesktopEvent(reachability.project, device.id, server.id, "DNS", `${name} Resolve-DnsName 질의 시간 초과: ${reachability.message}`, "dropped");
      onProjectChange(nextProject, reachability.message);
      return `Resolve-DnsName : ${name} query timed out: ${reachability.message}`;
    }
    if (queryType === "PTR") {
      const reverse = server.config.dnsRecords.find((item) => item.value === name);
      if (!reverse) {
        const loggedProject = appendServerLog(reachability.project, server.id, "warning", `RESOLVE-DNSNAME PTR ${name} from ${device.label}: NXDOMAIN`);
        const nextProject = appendDesktopEvent(loggedProject, device.id, server.id, "DNS", `${name} Resolve-DnsName PTR 질의가 NXDOMAIN을 반환했습니다.`, "dropped");
        onProjectChange(nextProject, "PTR 레코드를 찾을 수 없습니다.");
        return `Resolve-DnsName : ${name} : DNS name does not exist`;
      }
      const loggedProject = appendServerLog(reachability.project, server.id, "info", `RESOLVE-DNSNAME PTR ${name} from ${device.label}: ${reverse.name}`);
      onProjectChange(appendDesktopEvent(loggedProject, device.id, server.id, "DNS", `${name}을(를) ${reverse.name}(으)로 Resolve-DnsName 역조회했습니다.`, "delivered"), `DNS가 ${name}을(를) 역조회했습니다.`);
      return desktopResolveDnsNameOutput({ queryName: name, queryType, recordValue: reverse.name, dnsServerIp, serverLabel: server.label });
    }
    const record = server.config.dnsRecords.find((item) => item.name.toLowerCase() === name.toLowerCase());
    if (!record) {
      const loggedProject = appendServerLog(reachability.project, server.id, "warning", `RESOLVE-DNSNAME A ${name} from ${device.label}: NXDOMAIN`);
      const nextProject = appendDesktopEvent(loggedProject, device.id, server.id, "DNS", `${name} Resolve-DnsName A 질의가 NXDOMAIN을 반환했습니다.`, "dropped");
      onProjectChange(nextProject, "DNS 레코드를 찾을 수 없습니다.");
      return `Resolve-DnsName : ${name} : DNS name does not exist`;
    }
    const loggedProject = appendServerLog(reachability.project, server.id, "info", `RESOLVE-DNSNAME A ${record.name} from ${device.label}: ${record.value}`);
    onProjectChange(appendDesktopEvent(loggedProject, device.id, server.id, "DNS", `${record.name}을(를) ${record.value}(으)로 Resolve-DnsName 확인했습니다.`, "delivered"), `DNS가 ${record.name}을(를) 확인했습니다.`);
    return desktopResolveDnsNameOutput({ queryName: record.name, queryType, recordValue: record.value, dnsServerIp, serverLabel: server.label });
  }
  ...
  const webRequest = parseDesktopWebRequestCommand(command);
  if (webRequest.valid) {
    if (!webRequest.targetText.trim()) return "사용법: Invoke-WebRequest [-Uri] <url>";
    if (webRequest.method !== "GET") return `Invoke-WebRequest : Method ${webRequest.method} is not supported in this Desktop simulation.`;
    const resolved = await resolveDesktopNetworkTarget(project, device, webRequest.targetText, onProjectChange);
    if (!resolved.target) return `Invoke-WebRequest : ${resolved.error}`;
    const { target, project: resolvedProject } = resolved;
    const result = await simulatePing(resolvedProject, device.id, target.id, "http");
    if (!result.success) {
      onProjectChange(appendDesktopEvent(result.project, device.id, target.id, "HTTP", `Invoke-WebRequest 실패: ${result.message}`, "dropped"), result.message);
      return `Invoke-WebRequest : HTTP request failed: ${result.message}`;
    }
    if (!target.config.services.http) {
      const nextProject = appendDesktopEvent(result.project, device.id, target.id, "HTTP", `${target.label}이(가) Invoke-WebRequest 연결을 거부했습니다.`, "dropped");
      onProjectChange(nextProject, `${target.label}이(가) HTTP 연결을 거부했습니다.`);
      return `Invoke-WebRequest : Unable to connect to ${target.label}: HTTP service is stopped.`;
    }
    const loggedProject = appendServerLog(result.project, target.id, "info", `HTTP Invoke-WebRequest GET from ${device.label}`);
    onProjectChange(appendDesktopEvent(loggedProject, device.id, target.id, "HTTP", `Invoke-WebRequest ${target.label} 요청이 200 OK를 반환했습니다.`, "delivered"), "HTTP 200 OK.");
    const content = `${target.label} 웹 서비스가 실행 중입니다.`;
    return [
      "StatusCode        : 200",
      "StatusDescription : OK",
      `Content           : ${content}`,
      `RawContentLength  : ${content.length}`,
      `Headers           : {[Server, ${target.label}], [Content-Type, text/plain]}`
    ].join("\n");
  }
  ...
  return "알 수 없는 데스크톱 명령입니다. help, hostname, getmac [/v], ipconfig, netsh interface ip show config, Get-NetAdapter [-Name adapter], Get-NetIPConfiguration [-All] [-InterfaceAlias adapter], Get-DnsClientServerAddress [-AddressFamily IPv4], Get-NetRoute [-DestinationPrefix prefix], Get-NetNeighbor [-IPAddress ip], arp -a, arp -d <ip|*>, route print [-4], netstat -r|-rn, netstat -an, netstat -ano, netstat -abno, tasklist [/svc] [/fi \"PID eq <pid>\"], Get-NetTCPConnection [-State Listen] [-LocalPort port], Get-Process [-Id pid|-Name name], Get-Service [service], sc query[ex] [service], Test-NetConnection <ip|이름> [-Port 포트], Test-Connection <ip|이름> [-Count 횟수], ping [-4] [-n 횟수] <ip|이름>, tracert [-d] <ip|이름>, pathping [-n] <ip|이름>, nslookup [-type=A|PTR] <이름|ip> [dns-server], Resolve-DnsName <이름|ip> [-Server dns] [-Type A|PTR], Invoke-WebRequest [-Uri] <url>, http/web <ip|이름>, ftp <ip|이름>, email/mail <ip|이름> <받는사람>, ssh [-l user] [-p 22] <ip|이름>, telnet <ip|이름> [23], tftp <ip|이름>, syslog <ip|이름> <메시지>를 사용하세요.";
}
