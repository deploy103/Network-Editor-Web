...
async function desktopCommand(project: NetworkProject, device: NetworkDevice, command: string, onProjectChange: (project: NetworkProject, message: string) => void): Promise<string> {
  ...
  if (lower === "help" || lower === "?") {
    return [
      "지원 명령:",
      "  hostname | systeminfo | Get-ComputerInfo | getmac [/v]",
  ...
  if (lower === "systeminfo") return desktopSystemInfo(device);
  const getComputerInfoCommand = parseDesktopGetComputerInfoCommand(command);
  if (getComputerInfoCommand.valid) return desktopGetComputerInfo(device);
  ...
  const getEventLogCommand = parseDesktopGetEventLogCommand(command);
  if (getEventLogCommand.valid) return desktopGetEventLog(device, { newest: getEventLogCommand.newest, sourceFilter: getEventLogCommand.sourceFilter });
