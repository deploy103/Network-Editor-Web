...
- Desktop apps and commands for IP configuration, Command Prompt, console-cable Terminal, Web Browser, FTP, Email, TFTP, Syslog, `help`, `hostname`, `getmac`, `getmac /v`, `ipconfig`, `ipconfig /displaydns`, `ipconfig /flushdns`, `arp -a`, `arp -d`, `route print`, `route print -4`, `netstat -r`, `netstat -rn`, `netstat -an`, `netstat -ano`, `ping -4 -n`, `tracert -d`, `pathping -n`, directed `nslookup [-type=A|PTR] <name> [server]`, `http`/`web`, `ftp`, `email`/`mail`, `ssh`, `telnet`, `tftp`, and `syslog` checks.
