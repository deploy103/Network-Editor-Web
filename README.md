...
- Desktop apps and commands for IP configuration, Command Prompt, console-cable Terminal, Web Browser, FTP, Email, TFTP, Syslog, `help`, `hostname`, `getmac`, `getmac /v`, `ipconfig`, `ipconfig /displaydns` A/PTR cache output, `ipconfig /flushdns`, slash/dash `arp -a`/`arp -d`, `route print -4`, `netstat -r`/`-rn`, spaced or combined `netstat -an`/`-ano`, `ping -4 -n`, `tracert -d`, `pathping -n`, directed `nslookup [-type=A|PTR] <name> [server]`, `http`/`web`, `ftp`, `email`/`mail`, `ssh [-l user] [-p 22]`, `telnet <host> [23]`, `tftp`, and `syslog` checks.
